<script lang="ts">
  /**
   * Small icon set for report sections. Pure inline SVG, color-controlled via
   * `currentColor` so we can paint with Tailwind text-* utilities.
   */
  export let name:
    | 'spark'
    | 'shield'
...
>
  ...
  {:else if name === 'compass'}
    ...
    <path d="M15.5 8.5l-2 5-5 2 2-5 5-2z" fill="currentColor" stroke="none" opacity="0.85" />
  {:else if name === 'pin'}
  ...
  {:else if name === 'chart'}
    <path d="M3 20h18M5 17V8M10 17v-6M15 17v-9M20 17v-4" />
