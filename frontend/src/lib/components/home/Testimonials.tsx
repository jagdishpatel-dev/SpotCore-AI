  ...
  return (
    <section aria-labelledby="testimonials-heading" ref={ref}>
      <div className="geo-section">
        ...
        <h2 id="testimonials-heading" className="geo-section-title mt-3">Teams evaluating sites with SpotCore</h2>
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {quotes.map((q) => (
            <blockquote key={q.name} className="reveal-init geo-card flex h-full flex-col p-5" data-reveal-child>
              <p className="flex-1 text-sm leading-relaxed text-spotcore-text">&quot;{q.quote}&quot;</p>
              <footer className="mt-5 border-t border-spotcore-border pt-4">
                <p className="text-sm font-semibold text-spotcore-text">{q.name}</p>
                <p className="text-xs text-spotcore-text-muted">{q.role}</p>
              </footer>
            </blockquote>
          ))}
        </div>
