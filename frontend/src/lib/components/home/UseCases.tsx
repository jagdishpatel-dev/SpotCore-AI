        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2">
          {cases.map((c) => (
            <article key={c.title} className="reveal-init geo-card p-5" data-reveal-child>
              <h3 className="text-lg font-semibold text-spotcore-text">{c.title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-spotcore-text-muted">{c.body}</p>
              <a href={startHref} className="mt-4 inline-block text-sm font-semibold text-spotcore-accent hover:text-spotcore-accent-hover">Start analyzing →</a>
            </article>
          ))}
        </div>
      </div>
