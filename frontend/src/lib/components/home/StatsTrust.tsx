...
export default function StatsTrust() {
  ...
  return (
    <section className="border-y border-geoscorer-border bg-geoscorer-surface/60" aria-label="Trust metrics" ref={ref}>
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-6">
          {stats.map((s) => (
            <div key={s.label} className="reveal-init geo-metric-card text-center md:text-left" data-reveal-child>
              <p className="font-sans text-2xl font-medium tabular-nums tracking-tight text-geoscorer-accent md:text-3xl">{s.value}</p>
              <p className="mt-1 text-sm text-geoscorer-text-muted">{s.label}</p>
            </div>
          ))}
        </div>
      </div>
