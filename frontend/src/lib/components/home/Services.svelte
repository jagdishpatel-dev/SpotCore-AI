<script lang="ts">
  ...
  import ServicesAccordion, {
    type ServiceItem,
  } from '$lib/components/ui/ServicesAccordion.svelte';
  import { reveal } from '$lib/actions/reveal';

  /**
   * Services / "How GeoScore helps" — premium use-case accordion.
   *
   * Reword copy by editing the `items` array below. Each item maps to one
   * accordion row: { id, title, subtitle, body, bullets[] }.
   */
...
<section
  id="services"
  class="home-section relative overflow-hidden bg-[var(--bg-surface)]/40 py-24 md:py-28"
  aria-labelledby="services-heading"
>
  <!-- subtle top glow to anchor the band -->
  <div
    aria-hidden="true"
    class="pointer-events-none absolute inset-x-0 top-0 -z-10 h-[260px]"
    style="background: radial-gradient(900px 240px at 50% 0%, var(--glow-cyan), transparent 70%);"
  ></div>

  <div class="mx-auto max-w-7xl px-6 lg:px-10">
    <!-- Heading column: eyebrow + heading + subtext fade in together -->
    <div
      class="reveal-init relative mx-auto max-w-3xl text-center"
      use:reveal
    >
      <!-- soft halo behind the heading -->
      <div
        aria-hidden="true"
        class="pointer-events-none absolute left-1/2 top-1/2 -z-10 h-[240px] w-[520px] -translate-x-1/2 -translate-y-1/2 rounded-full opacity-50 blur-3xl"
      ...
      <h2
        id="services-heading"
        class="mt-5 font-display text-[40px] font-bold leading-[1.05] tracking-[-0.02em] text-text-primary sm:text-[52px] md:text-[60px] lg:text-[64px]"
      >
        Turn location questions into
        <span class="block shimmer-text">clear answers.</span>
      </h2>
      <p class="mx-auto mt-6 max-w-2xl text-base text-text-secondary md:text-lg">
        Different teams use GeoScore in different ways. Explore the core use
        cases we support out of the box.
      </p>
    </div>

    <!-- Accordion column: the accordion handles its own staggered reveal -->
    <div class="mx-auto mt-12 max-w-[1040px] md:mt-14">
      <ServicesAccordion {items} defaultOpen="market-entry" />
    </div>
  </div>
</section>
