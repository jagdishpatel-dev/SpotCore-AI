...
const steps = [
  { n: '01', title: 'Describe the site', body: 'Enter an address, business type, and trade area. GeoScorer frames the decision you are about to make.' },
...
export default function HowItWorks() {
  ...
  return (
    <section id="how-it-works" className="scroll-mt-24 bg-geoscorer-surface-soft" aria-labelledby="how-heading" ref={ref}>
      <div className="geo-section">
        <p className="geo-label">Process</p>
        <h2 id="how-heading" className="geo-section-title mt-3">How GeoScorer works</h2>
        <p className="mt-3 max-w-xl text-base text-geoscorer-text-muted">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {steps.map((step) => (
            <article key={step.n} className="reveal-init geo-card p-5 hover:!translate-y-0" data-reveal-child>
              <p className="font-sans text-sm font-medium tabular-nums text-geoscorer-accent">{step.n}</p>
              <h3 className="mt-3 text-lg font-medium tracking-heading text-geoscorer-text">{step.title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-geoscorer-text-muted">{step.body}</p>
            </article>
          ))}
        </div>
      </div>
